...
namespace SignalRDemo.UI
{
    ...
    {
        /// <summary>
        /// 定义一个connectionId的列表，如果MyHub是单例的话，可以不加static
        /// </summary>
        ...
        {
            connectionIdList.Add(this.Context.ConnectionId);
            Clients.All.refreshConnectionIds(connectionIdList.ToArray());
        ...
        {
            connectionIdList.Remove(this.Context.ConnectionId);
            Clients.All.refreshConnectionIds(connectionIdList.ToArray());
            string group = this.Context.QueryString["group"] ?? "";
